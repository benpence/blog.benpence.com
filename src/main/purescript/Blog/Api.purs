...
type Client =
  ...
  , about       :: forall r. Aff (ajax :: AJAX | r) (Result (Array Component))
  }
...
remoteAbout :: forall r. Aff (ajax :: AJAX | r) (Result (Array Component))
remoteAbout =
  let
    path = P.rootDir </> P.dir "api" </> P.file "about"

    url = Http.path (Right path) []
  in do
    resp <- Http.request Method.GET url Nothing
    pure (map (map toComponent) (decodeResult (resp.response)))
